...
-- Copyright 2017-2020 Signal Sciences Corp. All Right Reserved
...
sigsci = {
  ...
  -- user supplied response header will be added to 406/blocking responses if set
  extra_blocking_resp_hdr = "",

  -- user supplied custom content-types
  expected_content_types = nil,
}
...
local function valid_content_type (val)
  ...
  if string_starts_with(val, "application/graphql") then
    return true
  end
  ...
  if sigsci.expected_content_types ~= nil then
    local ect = sigsci.expected_content_types
    for i = 1, #ect do
      if string_starts_with(val, ect[i]) then
        return true
      end
    end
  end
...
local function get_rdata (txn, hin)

  local nowmillis = now_millis()

  -- http://cbonte.github.io/haproxy-dconv/1.7/configuration.html#7.3.4-ssl_fc
  -- https://www.arpalert.org/haproxy-lua-samples.html#simplify-complex-conditions
  local scheme
  if txn.f:ssl_fc() == 1 then
    scheme = "https"
  else
    scheme = "http"
  end
...
local function sigsci_prerequest(txn)
  local ctx
  local hin = transform_headers(txn.http:req_get_headers())
  local rdata = get_rdata(txn, hin)
  ...
  local postbody = nil
  local lenok = valid_content_length(content_len, sigsci.maxpost)
  if lenok and (valid_content_type(content_type) or txn.f:req_fhdr_cnt('content-type') > 1) then
  ...
  if istable(request_headers) then
    for i = 1, #request_headers do
      ...
      if v[1]:lower() == "x-sigsci-redirect" then
        redirect_location = v[2]
      else
        -- adds headers to original request
        txn.http:req_add_header(v[1], v[2])
  ...
  if waf_response >= 300 and waf_response <= 599 then
    -- blocking case
    debug("blocking with status %d for haproxy-version %s", waf_response, sigsci.server_version)
    -- if server version is > 1.8 then add the header and return
    txn:set_var('txn.sigsci_block', true)
    if sigsci.old_vers == false then
      return
    end
    -- else this is 1.7, or 1.8

    local ret = txn.res:set("") -- hap-1.9 wont let us do this
    if ret == -1 then
      debug("cannot reset body: status=%d", waf_response)
    end

    local body = "HTTP/1.1 ".. waf_response .. " Not Acceptable\r\nContent-Length: 19\r\n" ..
    "Content-Type: text/plain; charset=utf-8\r\n\r\n"..waf_response .." Not Acceptable\n"
    if sigsci.extra_blocking_resp_hdr ~= "" then
      body = "HTTP/1.1 ".. waf_response .. " Not Acceptable\r\nContent-Length: 19\r\n" ..
      sigsci.extra_blocking_resp_hdr .. "\r\nContent-Type: text/plain; charset=utf-8\r\n\r\n".. waf_response ..
      " Not Acceptable\n"
    end

    if waf_response <= 399 and string.len(redirect_location) > 0 then
      -- redirect case codes 300 - 399
      body = "HTTP/1.1 ".. waf_response .. " Moved\r\nLocation:" .. redirect_location ..
      "\r\nContent-Length: 19\r\n" .. "\r\nContent-Type: text/plain; charset=utf-8\r\n\r\n".. waf_response ..
      " Not Acceptable\n"
    end

    ret = txn.res:send(body) -- hap-1.9 wont let us do this
    debug("sent body with %s: ret=%d", waf_response, ret)
    if ret == -1 then
      internal_error("Failed to send waf_response: attempt retry", waf_response)
      txn.res:set(body)
...
local function split(str, sep)
  ...
  local first = string.sub(str, 1, idx-1)
  first = first:gsub("^%s*", "") -- trim space
  table.insert(words, first)
  local second = string.sub(str, idx+1)
  second = second:gsub("^%s*", "") -- trim space
  table.insert(words, second)
  return words
end
...
local function sigsci_init()
  ...
  sigsci.old_vers = sigsci.server_version:sub(1, #"1.8") == "1.8" or sigsci.server_version:sub(1, #"1.7") == "1.7"
end
...
core.register_service("sigsci_send_block", "http", function(applet)
    local msg = '{"status":406,"message":"Not Acceptable"}'
    -- TODO future support of alternative block code may be added instead of hard-coded 406
    applet:set_status(406)
    applet:add_header("content-length", string.len(msg))
    applet:add_header("content-type", "application/json")
    if sigsci.extra_blocking_resp_hdr ~= "" then
      -- split the header - handle multiple headers in the string
      local result = split(sigsci.extra_blocking_resp_hdr, '\r\n')
      if #result > 0 then
        for i = 1, #result do
          local hdr = result[i]
          local res = split(hdr, ':')
          if #res> 1 then
            applet:add_header(res[1], res[2])
          end
        end
      end
    end
